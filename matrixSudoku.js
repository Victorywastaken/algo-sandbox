...
function isSubmatrixFull(numbers){
  const result = [];
  let i = 0;
  while(i < numbers[0].length - 2){
    let sudokuMap = new Map();
    //creating a map with unique values 1 to 9
    for(let num = 1; num <= 9; num++){
      sudokuMap.set(num, true);
    }
    //sliding window of size 3x3
    for(let j = 0; j < numbers.length; j++){
      for(let k = i; k < i + 3; k++){
        if(sudokuMap.has(numbers[j][k])){
          sudokuMap.delete(numbers[j][k]);
        }
      }
    }
    if(sudokuMap.size === 0){
